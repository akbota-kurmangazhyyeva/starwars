...
const Intro: React.FC = () => {
  return (
    <div className="bg-block1 bg-cover bg-no-repeat h-screen p-8">
      <div className="flex flex-col gap-8 md:gap-12 lg:gap-16">
        ...
        <div className="grid grid-cols-2 md:gap-16 lg:gap-4 gap-4 sm:gap-16 md:grid-cols-3 lg:grid-cols-6 justify-items-center">
          {icons.map((icon) => (
            <Link href="/" key={icon.url} className="block w-20 h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 transform hover:scale-110 transition duration-300">
                <Image
                  src={icon.url}
                  alt={icon.name}
                  layout="responsive"
